...
import com.incture.pmc.util.MessageUIDto;
import com.incture.pmc.util.NoResultFault;
import com.incture.pmc.util.NonUniqueRecordFault;
import com.incture.pmc.util.RecordExistFault;
import com.incture.pmc.util.ServicesUtil;
import com.incture.pmc.workbox.dto.BaseDto;
import com.incture.pmc.workbox.entity.BaseDo;
...
public abstract class BaseDao<E extends BaseDo, D extends BaseDto>  {
	 ...
	 * @throws NoResultFault
	 */
	public D update(D dto) throws ExecutionFault, InvalidInputFault,
			NoResultFault {
	...
	protected E find(E pojo) throws ExecutionFault, NoResultFault {
		E result = null;
		try {
			result = (E) getEntityManager().find(pojo.getClass(),
					pojo.getPrimaryKey());
		} catch (Exception e) {
	...
	// while (getEntityManager().find(classDo, uuid) != null) {
	// if (i++ < noOfRetries) {
	...
	// queryName, pk));
	// }
	// E entity = getEntityManager().find(className, pk);
	// if (entity == null) {
	// throw new NoResultFault(ServicesUtil.buildNoRecordMessage(
	// queryName, pk));
	// }
